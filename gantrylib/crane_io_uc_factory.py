...
class CraneIOUCFactory:
    ...
    @classmethod
    def create_crane_io_uc(config: dict) -> CraneIOUC:
        """
        Factory function to create a CraneIOUC instance.

        Args:
            config (dict): Configuration dictionary. Should contain a key 'type' with value 'mock' or 'serial'.
                        For 'serial', must also contain 'CraneIOUCPort' and 'CraneIOUCBaudrate'.

        Returns:
            CraneIOUC: An instance of MockCraneIOUC or SerialCraneIOUC.
        """
        io_type = config.get("type", None).lower()
